...
def calculate_total_sphere_potential(positions, f, L):
    # gets one 2D numpy array of vectors (vector=position) (vectors in rows) as input, returns scalar equals to sum
    # sphere potentials for all particles
...
def calculate_interaction_potential(ri, rj, epsilon, R):
    ...
    VP_ij = epsilon * ((R/rij_norm)**12. - 2.*(R/rij_norm)**6.)
    return VP_ij
...
def calculate_total_potential_energy(positions, epsilon, R, f, L):
    ...
    total_potential_energy = calculate_total_sphere_potential(positions, f, L) +\
